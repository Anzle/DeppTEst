...
vardcl	: idlist ':' type { IDInfoPtr idinfoPtr = $1;
                            if (idinfoPtr != NULL) {
			      insert(idinfoPtr->IDstring,
				     $3.type,
                                     NextOffset($3.overallSize),
                                     $3.rowSize);
                              while (idinfoPtr->next != NULL) {
				insert(idinfoPtr->next->IDstring, 
  				       $3.type,
				       NextOffset($3.overallSize), /* NextOffset converts to byte addr. */
				       $3.rowSize);            /* This is in int units, not bytes!. */
...
idlist	: idlist ',' ID { char *temp = (char *) malloc (strlen($3.str)+1);
	...
	| ID		{ char *temp = (char *) malloc (strlen($1.str)+1);
                          strcpy(temp, $1.str);
...
type	: ARRAY '[' ICONST ']' OF stype { $$.overallSize = $3.num;
	                                  $$.type = $6.type;   
                                          $$.rowSize = 0; }
        | stype { $$.overallSize = $1.overallSize;
	          $$.type = $1.type;
                  $$.rowSize = $1.rowSize;}
	;

stype	: INT { $$.type = TYPE_INT; $$.overallSize = 1; $$.rowSize = -1; }
        | BOOL { $$.type = TYPE_BOOL; $$.overallSize = 1; $$.rowSize = -1; }
	;
...
lhs	: ID			{ SymTabEntry *entry = lookup($1.str);
				  ...
				  if (entry == NULL) 
				    printf("\n*** ERROR ***: Variable %s not declared.\n", $1.str);
				      ...
				      printf("\n*** ERROR ***: Variable %s is not a scalar variable.\n", $1.str);
                                    ...
                                    $$.type = entry->type;
				    sprintf(CommentBuffer, "Load LHS value of variable \"%s\" at offset %d", 
	...
	sprintf(CommentBuffer, 
							"Load LHS value of array variable \"%s\" with based address %d", 
							entry->name, entry->offset);
	emitComment(CommentBuffer);
